...
learning_rate = 0.0001  # ใช้ learning rate ที่สูงขึ้น
epochs = 100  # เพิ่มจำนวน epochs
best_model_path = 'best_fine_tuned_2_vgg16_2_model.keras'
batch_size = 32
...
x = tf.keras.layers.Dropout(0.5)(x)  # ลด Dropout Rate
...
try:
    model.load_weights('best_fine_tuned_1_vgg16_2_model.keras')
...
early_stopping = EarlyStopping(monitor='val_accuracy', patience=10, restore_best_weights=True, verbose=1)  # เปลี่ยนเป็น monitor val_accuracy
